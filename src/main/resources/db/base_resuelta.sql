CREATE DATABASE delicia_eterna;
...
/*Listar el nombre de cada jefe de sucursal en base al id de la sucursal, rol colaborador = 1 jefe*/
SELECT SUCURSAL.nombre , COLABORADOR.nombre, COLABORADOR.primerApellido, COLABORADOR.segundoApellido
from SUCURSAL
INNER JOIN COLABORADOR
ON SUCURSAL.idSucursal = COLABORADOR.FK_idSucursal where COLABORADOR.rol = 1 ;


SELECT DISTINCT cl.nombre, cl.primerApellido, cl.segundoApellido, s.nombre
FROM CLIENTE cl
JOIN VENTA v ON cl.rutCliente = v.FK_rutCliente
JOIN SUCURSAL s ON v.FK_idSucursal = s.idSucursal
WHERE v.FK_idSucursal = 1;

/*Listar la disponibilidad de productos para una sucursal determinada entregando como
 información el nombre de la sucursal, el nombre del producto y el stock */
SELECT SUCURSAL.nombre AS "nombre Sucursal", PRODUCTO.nombreProducto, stock.stock
from STOCK
INNER JOIN SUCURSAL ON SUCURSAL.idSucursal = stock.idSucursal
...
FROM SUCURSAL s
JOIN STOCK st ON s.idSucursal = st.idSucursal
JOIN PRODUCTO p ON st.idProducto = p.idProducto
WHERE s.idSucursal = 5;

SELECT cl.nombre, cl.primerApellido, cl.segundoApellido, s.nombre, COUNT(v.idVenta) AS cantidadVentas
FROM CLIENTE cl
JOIN VENTA v ON cl.rutCliente = v.FK_rutCliente
JOIN SUCURSAL s ON v.FK_idSucursal = s.idSucursal
WHERE s.idSucursal = 2
GROUP BY cl.rutCliente, s.idSucursal
